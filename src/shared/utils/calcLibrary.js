...
const calculateLoanPayment = ({
  ...
  const monthlyRate = roundMonthRate({ value: annualInterestRate / 100 / 12 });
...
const loansGetFhaTerm = (term, ltv, fhaCaseDate) => {
  let miYear = 0,
    miCancel = 0;
...
const calculateCashFlows = ({
  ...
  ratesArray.forEach((rate) => {
    ...
    let { monthlyPayment: payment } = calculateLoanPayment({
      annualInterestRate: noteRate * 100,
...
const handleCalculateARP = ({
  ...
  if (productCategory === 4) miYear = 0; //For VA
  ...
  while (miYearCount <= miYear) {
    ...
    if (productCategory === 1) {
      ...
      if (miMonthStart > 120) {
        miAmount = roundValue({ value: (loanAmount * 0.002) / 12 });
      }
